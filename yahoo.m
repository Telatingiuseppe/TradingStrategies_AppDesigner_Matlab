function [AdjClose, sigma] = yahoo(symbol, startdate, enddate)
    ...
    AdjClosecolumn = data.AdjClose(1:end);
    AdjClose = AdjClosecolumn(end,1);
    returns = diff(AdjClosecolumn)./(AdjClosecolumn(1:end-1,1));
    sigma = sqrt((var(returns))*252);
end